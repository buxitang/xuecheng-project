...
public class OrderServiceImpl implements OrderService {
    ...
    public XcPayRecord createPayRecord(XcOrders orders){
        ...
        if (insert <= 0){
            XueChengException.cast("插入支付记录失败");
        }
        return payRecord;
    ...
    public void saveAliPayStatus(PayStatusDto payStatusDto) {
        ...
        String states = payRecord.getStatus();
        if ("601002".equals(states)){
            return;
        }
        ...
        if (trade_status.equals("TRADE_SUCCESS")) {
            ...
            int update1 = payRecordMapper.update(payRecord, new LambdaQueryWrapper<XcPayRecord>().eq(XcPayRecord::getPayNo, payNo));
            if (update1 > 0) {
            ...
            int update = ordersMapper.update(orders, new LambdaQueryWrapper<XcOrders>().eq(XcOrders::getId, orderId));
            if (update > 0) {
